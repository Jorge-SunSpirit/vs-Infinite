...
class PlayState extends MusicBeatState
{
	...
	public var hasShadows:Bool = false;
	...
	private var healthBarBGUL:AttachedSprite;
	...
	override public function create()
	{
		...
		if (stageData.has_Shadows == true)
			hasShadows = stageData.has_Shadows;
		...
		dad = new Character(0, 0, SONG.player2, false, hasShadows);
		startCharacterPos(dad, true);
		dadGroup.add(dad);
		startCharacterLua(dad.curCharacter);

		boyfriend = new Boyfriend(0, 0, SONG.player1, hasShadows);
		...
		timeBar.createFilledBar(0xFF181818, 0xFFC80000);
		...
		healthBarBGUL = new AttachedSprite('healthBarUnderlay');
		healthBarBGUL.scrollFactor.set();
		healthBarBGUL.visible = !ClientPrefs.hideHud;
		healthBarBGUL.xAdd = -64;
		healthBarBGUL.yAdd = -68;
		add(healthBarBGUL);
		...
		healthBarBG.yAdd = -4;
		add(healthBarBG);
		healthBarBGUL.sprTracker = healthBarBG;
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && !inCutscene)
		{
			if(!cpuControlled) {
				keyShit();
			} else if(boyfriend.animation.curAnim != null && boyfriend.holdTimer > Conductor.stepCrochet * (0.0011 / FlxG.sound.music.pitch) * boyfriend.singDuration && boyfriend.singing && !boyfriend.animation.curAnim.name.endsWith('miss')) {
				boyfriend.dance();
	...
	function playAnimNote(char:Character, note:Note, ?altAnim:String = "")
	{
		...
		char.holding = note.isSustainNote && !note.isSustainEnd();
		...
		if (char != null)
		{
			...
			{
				if (!note.isSustainNote || char.holding)
				{
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		StrumPlayAnim(true, Std.int(Math.abs(note.noteData)), time, dad);
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (cpuControlled)
			{
				...
				StrumPlayAnim(false, Std.int(Math.abs(note.noteData)), time, char);
			} 
			else
			{
	...
	public function spawnNoteSplashOnNote(note:Note) {
		if(ClientPrefs.noteSplashes && note != null) {
			...
			if(strum != null) {
				spawnNoteSplash(strum.x + 5, strum.y + 25, note.noteData, note);
			}
		}
	}
